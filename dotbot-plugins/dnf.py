...
class Apt(Plugin):
    ...
    def handle(
        ...
        success = True
        dnf_executable = which("dnf")
        if dnf_executable is None:
            self._log.error("dnf executable not found")
            return False
    ...
    def _install(
        ...
        *,
        ...
        self._log.info(f"Installing {', '.join(packages)} dnf packages")
        args: list[str] = [
            "sudo",
            dnf_executable,
            "install" if not reinstall else "reinstall",
            "--assumeyes",
        ]
